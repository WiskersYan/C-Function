...
可以根据他们如何比较词典相互比较来排序不同的排列;第一个这样排序的可能排列（将字典比较小于所有其他排列的那个排列）
是使其所有元素按升序排序的排序，并且最大排列的所有元素按降序排序。

各个元素的比较使用operator <用于第一个版本，或comp用于第二个版本。

如果函数可以确定先前的排列，则它会重新排列元素并返回true。如果那是不可能的
（因为它已经处于最低可能的排列），它会根据最后的排列重新排列元素（按降序排序）并返回false。
参数
...
如果函数可以将对象重新排列为lexicographicaly较小的排列，则为true。
否则，该函数返回false以指示排列不小于前一个，但最大可能（按降序排序）。
*/
